...
genius_tracklist <- function(artist = NULL, album = NULL) {
  ...
  # Get the album name
  album_name <- html_nodes(genius_session, ".header_with_cover_art-primary_info-title") %>%
    html_text()

  # Get track numbers
  # Where there are no track numbers, it isn't a song
  track_numbers <- html_nodes(genius_session, ".chart_row-number_container-number") %>%
    html_text() %>%
    str_replace_all("\n", "") %>%
    str_trim()
  ...
  track_titles <- html_nodes(genius_session, ".chart_row-content-title") %>%
    html_text() %>%
    str_replace_all("\n","") %>%
  ...
  # Get all song urls
  track_url <- html_nodes(genius_session, ".u-display_block") %>%
    html_attr('href') %>%
    str_replace_all("\n", "") %>%
    str_trim()
